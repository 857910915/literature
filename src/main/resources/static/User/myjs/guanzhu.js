...
function toShow() {
    $.get("../follow/selectFollowList",{"username":username,"pageNo":pageNo},function (data) {
        console.log(data);
        for (var i in data){
            var fol=data[i];
            var user=fol.user;
            $("#guanzhu1").append('<dl class="lastdl">\n' +
                '                                    <dt>\n' +
                ...
                '                                        </div>\n' +
                '                                        <div class="opera">\n' +
                '                                            <a href="javascript:;" class="operass space_atten_sp" data-mid="657491">已关注</a>\n' +
